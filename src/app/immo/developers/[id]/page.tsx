...
export default async function DeveloperDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <main className="min-h-screen bg-[#FAFAFC] pb-32 overflow-hidden">
         ...
         <div className="absolute bottom-0 left-0 right-0 px-4 translate-y-1/2">
            <div className="container mx-auto max-w-6xl">
               <Reveal delay={0.2} direction="up">
                   <div className="glass-panel rounded-[2.5rem] p-8 md:p-12 flex flex-col md:flex-row items-center md:items-end justify-between gap-8 hover:shadow-[0_20px_60px_rgb(79,70,229,0.15)] transition-all duration-700">
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
               ...
               <Reveal key={i} delay={0.4 + (i * 0.1)}>
                   <div className={"p-8 rounded-3xl border transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl bg-white " + stat.border}>
         ...
         <div className="pt-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               {projects.length > 0 ? projects.map((p, idx) => (
                  <Reveal key={p.id} delay={0.2 + (idx * 0.1)}>
