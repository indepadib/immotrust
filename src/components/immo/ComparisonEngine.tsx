             ...
             <div key={p.id} className="space-y-8 p-10 bg-white/5 rounded-[2.5rem] border border-white/10 hover:border-primary/30 transition-all relative overflow-hidden group/card">
                ...
                <div className="flex items-start justify-between">
                   <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                         <Shield className={clsx("w-3 h-3", p.trustScore > 80 ? "text-emerald-500" : "text-amber-500")} />
                         {p.developerId}
                      </div>
                   </div>
                   <ScoreBadge score={p.trustScore} size="lg" />
                </div>
                ...
                <div className="space-y-6">
                   ...
                   <div className={clsx(
                     ...
                     compareMetric(p.pricePerMeter, projects[1-idx].pricePerMeter, true) === 'win' 
                      ...
                      <div className="text-2xl font-black text-white italic">{p.pricePerMeter.toLocaleString()} DH</div>
                   </div>

                   <div className={clsx(
                     "p-6 rounded-2xl border transition-all",
                     p.delayRisk === 'low' ? "bg-emerald-500/10 border-emerald-500/20" : "bg-amber-500/10 border-amber-500/20"
                   )}>
                      <div className="flex items-center justify-between mb-2">
                         <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Risque Délai</span>
                         <Clock className="w-4 h-4 text-primary" />
                      </div>
                      <div className="text-xl font-black text-white uppercase italic tracking-widest">{p.delayRisk}</div>
                   </div>
          ...
          <div className="flex items-start gap-6">
             ...
             <div>
                ...
                <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest leading-relaxed max-w-sm mt-1">
                   Le projet <span className="text-primary">{projects[0].trustScore > projects[1].trustScore ? projects[0].title : projects[1].title}</span> présente un meilleur ratio Risque/Conformité sur 42 points d'audit.
                </p>
             </div>
          </div>
